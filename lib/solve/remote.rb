...
# This class it the mirror of Solve::Connection::Local.  A Solve::Box which is
...
# the solve shell or library.
class Solve::Connection::Remote
	attr_reader :host, :tunnel

	def initialize(host)
		@host = host
		@tunnel = Solve::SshTunnel.new(host)
	end
	...
	# object), send it across the wire to the SolveServer listening on the other
	# side.  Uses http basic auth, with credentials fetched from the Solve::Config.
	def transmit(params)
		ensure_tunnel
	...
	def process_result(code, body)
		raise Solve::NotAuthorized if code == "401"
		...
		raise Solve::FailedTransmit if code != "200"
	...
	def parse_exception(body)
		...
		raise "invalid exception class: #{klass}" unless klass.match(/^Solve::[A-Za-z]+$/)
