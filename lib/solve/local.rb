...
# Solve::Box uses a connection object to execute all solve commands.  If the box
# is local, Solve::Connection::Local is created.  The local connection is the
# heart of solve's internals.  (Users of the solve shell or library need never
# access the connection object directly, so the docs herein are intended for
# developers wishing to modify solve.)
#
# The local connection has a series of methods which do the actual work of
# modifying files, getting process lists, and so on.  SolveServer creates a
...
# Solve::Connection::Local#receive.
class Solve::Connection::Local
	...
	def file_contents(full_path)
		...
		raise Solve::DoesNotExist, full_path
	end
	...
	def rename(path, name, new_name)
		raise(Solve::NameCannotContainSlash, "#{path} rename #{name} to #{new_name}") if new_name.match(/\//)
		old_full_path = "#{path}/#{name}"
		new_full_path = "#{path}/#{new_name}"
		raise(Solve::NameAlreadyExists, "#{path} rename #{name} to #{new_name}") if ::File.exists?(new_full_path)
	...
	def copy(src, dst)
		...
		raise Solve::DoesNotExist, File.dirname(dst)
	rescue RuntimeError
		raise Solve::DoesNotExist, src
	end
	...
	def read_archive(full_path)
		`cd #{Solve.quote(::File.dirname(full_path))}; tar c #{Solve.quote(::File.basename(full_path))}`
	end

	# Extract an in-memory archive to a dir.
	def write_archive(archive, dir)
		IO.popen("cd #{Solve::quote(dir)}; tar x", "w") do |p|
			p.write archive
		end
	end
	...
	def index(base_path, glob)
		...
		raise Solve::DoesNotExist, base_path
	end
	...
	def size(full_path)
		`du -sb #{Solve.quote(full_path)}`.match(/(\d+)/)[1].to_i
	end
	...
	# for use in the Solve::Process#new.
	...
	# suitable for use in the Solve::Process#new.
	def parse_ps(line)
		m = line.split(" ", 6)
		params = {}
		params[:pid] = m[0]
	...
	def bash(command, user=nil, background=false, reset_environment=false)
		...
		raise(Solve::BashFailed, err) if retval != 0
	...
	# Raised when the action passed in by SolveServer is not known.
	class UnknownAction < Exception; end

	# SolveServer uses this method to transform a hash (:action plus parameters
	...
	def receive(params)
		case params[:action]
			...
			when 'set_access'     then set_access(params[:full_path], Solve::Access.from_hash(params))
