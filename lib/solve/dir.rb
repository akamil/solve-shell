# A dir is a subclass of Solve::Entry that contains other entries.  Also known
# as a directory or a folder.
#
# Dirs can be operated on with Solve::Commands the same as an array of files.
...
class Solve::Dir < Solve::Entry
	...
	def find_by_name(name)    # :nodoc:
		Solve::Entry.factory("#{full_path}/#{name}", box)
	end

	def find_by_glob(glob)    # :nodoc:
		connection.index(full_path, glob).map do |fname|
			Solve::Entry.factory("#{full_path}/#{fname}", box)
		end
	end
	...
	def make_entries(filenames)
		filenames.map do |fname|
			Solve::Entry.factory("#{full_path}/#{fname}")
		end
	end
	...
	# Run a bash command starting in this directory.  Options are the same as Solve::Box#bash.
