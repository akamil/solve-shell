# The commands module contains operations against Solve::File entries, and is
# mixed in to Solve::Entry and Array.  This means you can run these commands against a single
...
module Solve::Commands
	# The entries command must return an array of Solve::Entry items.  This
	# varies by class that it is mixed in to.
	def entries
		raise "must define me in class mixed in to for command use"
	end

	# Search file contents for a regular expression.  A Solve::SearchResults
	# object is returned.
	def search(pattern)
		results = Solve::SearchResults.new(pattern)
