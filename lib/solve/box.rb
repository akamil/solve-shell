# A solve box is a single unix machine - a server, workstation, or VPS instance.
#
...
class Solve::Box
	...
	# the local one (e.g. Solve::Box.new('user@host')).
	...
	# Access / on the box.
	def filesystem
		Solve::Entry.factory('/', self)
	end

	# Look up an entry on the filesystem, e.g. box['/path/to/some/file'].
	# Returns a subclass of Solve::Entry - either Solve::Dir if you specifiy
	# trailing slash, or Solve::File otherwise.
	def [](key)
		filesystem[key]
	end

	# Get the list of processes running on the box, not unlike "ps aux" in bash.
	# Returns a Solve::ProcessSet.
	def processes
		Solve::ProcessSet.new(
			connection.processes.map do |ps|
				Solve::Process.new(ps, self)
			end
		)
	end

	# Execute a command in the standard unix shell.  Returns the contents of
	# stdout if successful, or raises Solve::BashFailed with the output of stderr
	# if the shell returned a non-zero value.  Options:
	#
	# :user => unix username to become via sudo
	# :env => hash of environment variables
	# :background => run in the background (returns Solve::Process instead of stdout)
	#
	# Examples:
	#
	...
	def make_connection    # :nodoc:
		host == 'localhost' ? Solve::Connection::Local.new : Solve::Connection::Remote.new(host)
	end
