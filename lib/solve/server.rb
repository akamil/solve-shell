...
# Solve::Connection::Local call.  The results are sent back across the wire to
# be decoded by Solve::Connection::Remote on the other side.
class SolveHandler < Mongrel::HttpHandler
	...
	def log(msg)
		File.open('solved.log', 'a') do |f|
			f.puts "#{Time.now.strftime('%Y-%m-%d %H:%I:%S')} :: #{msg}"
		end
	end
end

# A container class to run the Mongrel server for solved.
class SolveServer
	def run
		host = "127.0.0.1"
		port = Solve::Config::DefaultPort

		solved = SolveHandler.new
		solved.log "solved listening on #{host}:#{port}"
