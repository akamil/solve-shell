...
# See Solve::Entry#access= for information on the public-facing interface.
class Solve::Access
	...
	def extract_list(type, value, choices)
		...
		list.each do |value|
			raise(Solve::BadAccessSpecifier, "Unrecognized #{type}: #{value}") unless choices.include? value
		end
	end
