# Solve::Entry is the base class for Solve::File and Solve::Dir.  One or more of
...
class Solve::Entry
	...
	def initialize(full_path, box=nil)
		...
		@box = box || Solve::Box.new('localhost')
	end

	# The factory checks to see if the full path has a trailing slash for
	# creating a Solve::Dir rather than the default Solve::File.
	def self.factory(full_path, box=nil)
		if full_path.tail(1) == '/'
			Solve::Dir.new(full_path, box)
		elsif File.directory?(full_path)
			Solve::Dir.new(full_path, box)
		else
			Solve::File.new(full_path, box)
		end
	end
	...
	def connection
		box ? box.connection : Solve::Connection::Local.new
	end

	# The parent dir.  For example, box['/etc/hosts'].parent == box['etc/']
	def parent
		@parent ||= Solve::Dir.new(@path)
	end
	...
	def duplicate(new_name)
		raise Solve::NameCannotContainSlash if new_name.match(/\//)
	...
	# copy.
	def copy_to(dir)
		raise Solve::NotADir unless dir.class == Solve::Dir
	...
	#
	def access=(options)
		connection.set_access(full_path, Solve::Access.parse(options))
	end
	...
	#
	def access
		Solve::Access.new.from_octal(stat[:mode]).display_hash
	end
