# The config class accesses files in ~/.solve to load and save user preferences.
class Solve::Config
	DefaultPort = 7770

	attr_reader :dir

	# By default, reads from the dir ~/.solve, but an optional parameter allows
	# using another location.
	def initialize(location=nil)
		@dir = Solve::Dir.new(location || "#{ENV['HOME']}/.solve")
		@dir.create
	end
	...
	# or .profile.
	#
	# Example ~/.solve/env.rb:
	#
	#   server = Solve::Box.new('www@my.server')
	...
	# Commands are mixed in to Array and Solve::Entry, alongside the default
	# commands from Solve::Commands.  Any methods here should reference "entries"
	# to get the list of entries to operate on.
	#
	# Example ~/.solve/commands.rb:
	#
	...
	# remote access via solved.  You can fill this in manually, or let the remote
	...
	# ~/.solve/tunnels contains a list of previously created ssh tunnels.  The
